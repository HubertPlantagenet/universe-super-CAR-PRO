...
public class client{
	...
	//必须使用静态变量
	private ReanderAndSender mReaderAndSender = null;
	
	public client() {
		//do nothing
		}
	public client(ReanderAndSender mReaderAndSender) {
		this.mReaderAndSender = mReaderAndSender;
	}
	
	public void clientHandler() { //不能在一个静态方法中引用该类中的另外一个静态方法
		try {
			Socket sk = new Socket(serverIP,port);
			System.out.println("Connected");
			//开启读写线程
			new Thread(new WriteHandler(sk)).start();
			if(mReaderAndSender == null)
				new Thread(new ReadHandler(sk)).start();
			else
				new Thread(new CommReadHandler(this.mReaderAndSender,sk));
		} catch (Exception e) {
...
class CommReadHandler implements Runnable{
	private Socket sk;
	private ReanderAndSender mReaderAndSender;
	public CommReadHandler(ReanderAndSender mReaderAndSender,Socket sk) {this.sk = sk;this.mReaderAndSender = mReaderAndSender;}
	@Override
	public void run() {
		BufferedReader in = null;
		try {
